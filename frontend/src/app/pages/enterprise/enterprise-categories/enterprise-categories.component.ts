...
@Component({
  ...
  styleUrls: ['./enterprise-categories.component.css'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class EnterpriseCategoriesComponent implements OnInit {
  categories = signal<EnterpriseCategory[]>([]);
  filteredCategories = signal<EnterpriseCategory[]>([]);
  ...
  openEditForm(category: EnterpriseCategory) {
  ...
  submitForm() {
    ...
    const request = this.isEditing()
      ? this.enterpriseCategoryService.update(this.editingId()!, formData)
      : this.enterpriseCategoryService.create(formData);
  ...
  deleteCategory(category: EnterpriseCategory) {
    Swal.fire({
      title: '¿Estás seguro?',
      ...
      if (result.isConfirmed) {
        this.enterpriseCategoryService.delete(category.id).subscribe({
          next: () => {
            this.loadCategories();
