...
export const createPaymentIntentSchema = {
    body: S.object()
        ...
        .prop('products', S.array()
            .items(
                S.object()
                    .prop('id', S.string().required().description('Product ID'))
                    .prop('name', S.string().required().description('Product name'))
                    .prop('price', S.number().required().description('Product price'))
                    .prop('quantity', S.integer().minimum(1).required().description('Quantity'))
            )
            .description('Array of product items to purchase')
        )
